...
expand.grid(sequential = c(TRUE, FALSE), allow_invalid = c(TRUE, FALSE),
            leave_bounds = c(TRUE, FALSE), track = c(TRUE, FALSE),
            resolution = c(1000, 2000, 4000))
...
# Test the following args below
# Sequential
# Allow empties
# Track
for(i in 1:10) {
  out <- simrabid(start_up, start_vacc = 0.2, I_seeds = 5, vacc_dt,
                  params = list(R0 = 1.5, k = 100, iota = 4),
                  ...
                  leave_bounds = TRUE, max_tries = 5,
                  summary_funs = list(use_mget = use_mget),
                  track = FALSE,
                  row_probs = NULL)
}


# Also test if changing scales works

# Also test if changing functions works
